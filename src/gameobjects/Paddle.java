...
public class Paddle extends GameObject {
    ...
    public void update(float deltaTime) {
        ...
        float x = this.transform().getTopLeftCorner().x();
        float xLeft = x + movementDir.x();
        float xRight = xLeft + this.transform().getDimensions().x();
        if (inputListener.isKeyPressed(KeyEvent.VK_LEFT)) {
            if (!((xLeft < 0)&&(xLeft<(-1)*minDisFromEdge))) {
                movementDir = movementDir.add(Vector2.LEFT);
            }
        }
        if (inputListener.isKeyPressed(KeyEvent.VK_RIGHT)) {
            if (! ((xRight > windowDimensions.x())&&xRight-windowDimensions.x()>minDisFromEdge)) {
